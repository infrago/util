...
func NewHashRing(weights map[string]int, spotsArgs ...int) *HashRing {
	...
	return (&HashRing{spots: spots, weights: weights}).generate()
}
...
func (h *HashRing) generate() *HashRing {
	...
	for nodeKey, w := range h.weights {
		...
		for i := 1; i <= spots; i++ {
			hash := sha1.New()
			_, _ = hash.Write([]byte(nodeKey + ":" + strconv.Itoa(i)))
			hashBytes := hash.Sum(nil)
			n := hashringNode{nodeKey: nodeKey, spotValue: genValue(hashBytes[6:10])}
